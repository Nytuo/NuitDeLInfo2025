...
const missionOptions: ChatOption[] = [
    ...
    },
    {
        ...
        value: "donation",
        icon: "💰",
        description: "Soutenir financièrement la cause",
    },
    {
        label: "Devenir Bénévole",
        value: "volunteer",
        icon: "🛡️",
        description: "Rejoindre la guilde des héros",
    },
    {
        label: "Demander des Infos",
        value: "information",
        icon: "❓",
        description: "En savoir plus sur nos actions",
    },
];
...
async function startConversation() {
    ...
    addMessage({
        type: "bot",
        content:
            "Avant de commencer, puis-je connaître ton nom, noble visiteur ?",
    });
...
async function handleUserInput() {
    ...
    if (AIService.isSpam(input)) {
        addMessage({
            type: "bot",
            content:
                "🚫 Oups ! Ce message semble suspect. Peux-tu reformuler ?",
        });
        currentInput.value = "";
        return;
    }
        ...
        case 2: // Email
            if (!isValidEmail(input)) {
                addMessage({
                    type: "bot",
                    content:
                        "📧 Hmm, cet email ne semble pas valide. Peux-tu vérifier ?",
                });
                return;
            }
...
function selectMission(mission: MissionType) {
    ...
    addMessage({
        type: "user",
        content:
            missionOptions.find((m) => m.value === mission)?.label || mission,
    });
    ...
    switch (mission) {
        case "contact":
            addMessage({
                type: "bot",
                content:
                    "📞 Excellent choix ! Quel est le sujet de ton message ?",
            });
            break;
        ...
        case "volunteer":
            addMessage({
                type: "bot",
                content:
                    "🛡️ Un nouveau membre pour la Guilde ! Quelles sont tes compétences ?",
            });
            await simulateTyping(300);
            addMessage({
                type: "options",
        ...
        case "information":
            addMessage({
                type: "bot",
                content:
                    "📚 Quelle information recherches-tu ? Pose ta question !",
            });
            break;
    }
}
...
async function selectRecurrence(recurrence: string) {
    ...
    addMessage({
        type: "user",
        content: labels[recurrence] || recurrence,
    });
...
async function finishConversation() {
    ...
    if (props.widgetMode) {
        ...
        const confirmationMessage =
            AIService.generateConfirmationMessage(userData);
        addMessage({
            type: "bot",
...
function handleInputChange() {
    ...
    if (input.length > 5) {
        ...
        suggestions.value = AIService.generateSuggestions(
            input,
            userData.mission,
        );
    } else {
